...
-- Limit dihilangkan dengan angka sangat besar
getgenv().MAX_SPAWN_BEFORE_FORGET = 999999999 
...
local function setupCashWatcher()
	...
	cashValue:GetPropertyChangedSignal("Value"):Connect(function()
		if not getgenv().currentTarget then lastCash = cashValue.Value return end
		...
		if tgt and tgt.Parent then
			...
			if hum and hrp and part then
				...
				if not spawnTime or tick() - spawnTime >= getgenv().CHASE_DELAY then
					...
					if false then hum:MoveTo(part.Position) end
...
if not getgenv().__KAMI_APA_AUTO_SPEED_COIL then
	...
	player.CharacterAdded:Connect(function() task.wait(1) equipSpeedCoil() end)
	if player:FindFirstChildOfClass("Backpack") then
		player.Backpack.ChildAdded:Connect(function(tool) task.wait(0.2) equipSpeedCoil() end)
	end
	task.spawn(function() while true do equipSpeedCoil() task.wait(1) end end)
end
		...
		while getgenv().__KAMI_APA_ANTI_AFK do
			...
			if hum and hrp and hum.Health > 0 then
				...
				pcall(function()
					...
					if cam then cam.CFrame = cam.CFrame * CFrame.Angles(0, math.rad(math.random(-10,10)), 0) end
				end)
			end
